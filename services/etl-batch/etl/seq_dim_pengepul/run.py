...
# Main Sequence
@log.time_it_seq("Batch - Sequence dim_mitra_pengepul", sequence_logger)
def main():
    # Variables
    ...
    with beam.Pipeline() as pipeline:
        (
            pipeline
            | "Read DB - mitra_pengepul_luar" >> relational_db.ReadFromDB(
                source_config = connection.get_connection("ops-db"),
                table_name = "mitra_pengepul_luar",
                query = database.get_query(QUERY_DIR, "mitra_pengepul_luar")
            )
            | "Enrich System Data" >> beam.Map(database.enrich_system_data(datetime.now(TZINFO)))
            | "Load Data - dim_pengepul" >> relational_db.Write(
                source_config = connection.get_connection("dwh-db"),
                table_config = relational_db.TableConfiguration(
                    name = "dim_pengepul",
                    create_if_missing = True,
                    define_table_f = models.DimPengepul,
